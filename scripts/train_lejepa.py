...
from src.lejepa_transforms import LocalViewsCfg, build_local_views_transform,collate_views_with_meta
from src.lejepa_model import ProjectorCfg, MLPProjector, gap_pool
...
def main(args):
    ...
    dl = DataLoader(
        ...
        sampler=sampler,                 # IMPORTANT: sampler replaces shuffle
    ...
    while step < total_steps:
        ...
        for vs, _ in it:
            ...
            # ---- ONE-TIME VIEW SANITY CHECK (DELETE AFTER) ----
            if is_main and step == 0:
                v = vs[0].detach().float().cpu()  # (V, C, H, W)

                if bool(cfg["ssl"]["normalize_imagenet"]):
                ...
                grid = make_grid(v, nrow=v.shape[0], padding=4)
                to_pil_image(grid).save(rp.run_dir / "DEBUG_views_once.png")
            # ---- END DEBUG BLOCK ----
            bs, V, C, H, W = vs.shape
